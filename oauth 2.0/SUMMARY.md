...
- Проведён анализ OIDC-flow и JWT через браузерные инструменты и безопасные декодеры.
- Добавлено логирование событий login, token exchange и logout без записи самих токенов.
- Проверена обработка:
  - просроченного токена → 401;
  - неверного audience → 401;
  - отсутствия требуемой admin role → 403.
  - Подготовлен runbook с действиями при проблемах входа, проверке JWT и ротации ключей Keycloak.
